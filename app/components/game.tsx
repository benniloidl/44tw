...
export default function Game({ gameId }: { gameId: string }) {
    ...
    useEffect(() => {
        ...
        setGameUrl(`${window.location.protocol}//${window.location.hostname}${window.location.port ? ":" + window.location.port : "" }/${encodeURIComponent(gameId)}`);

        // Set up WebSocket connection
        const protocol = window.location.protocol == 'https:' ? 'wss' : 'ws';
        let ws_conn_url = `${protocol}://${window.location.hostname}:${window.location.port}/ws/?gameId=${encodeURIComponent(gameId)}`;
